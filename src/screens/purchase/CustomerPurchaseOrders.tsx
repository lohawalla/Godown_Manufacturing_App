    ...
    <SafeAreaView>
      <Navbar/>
        ...
        <View style={{padding:12, height:'78%'}}>
            <FlatList
            data={data?.data}
            renderItem={({item}:any):JSX.Element => <PartyListCard onPress={()=>next(item._id)} primaryImage='https://shorturl.at/dlpDG' qrImage='https://rb.gy/m8zvbd' DateTime={item.createdAt.substring(0, 10)} Name={item.partyId.partyName} status='pending' billNumber={item?.billNumber} salesNumber={item?.salesNumber} purchaseNumber={item?.purchaseNumber || '24542'}/>}
