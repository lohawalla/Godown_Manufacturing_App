import { Alert, Button, FlatList, Image, Pressable, SafeAreaView, StyleSheet, Text, Touchable, TouchableOpacity, View } from 'react-native'
...
const Assign = ({ navigation }: any) => {
    ...
    return (
        <SafeAreaView style={{ backgroundColor: 'white', flex: 1 }}>
            ...
            {show ?
                (<CodeScannerPage setShow={setShow} setScannedValue={setScannedValue} navigation={navigation} />) :
                (<View style={{ zIndex: 5 }}>
                    <View style={{ flexDirection: 'column', alignItems: 'center' }}>
                        <View style={{ marginVertical: 20 }}>
                            ...
                            {value.shelf !== 'Select' && (<SelectedCard value={value.shelf} handlePress={handlePress} />)}
                        </View>
                        <View style={{ marginVertical: 80, }}>
                            <TouchableOpacity disabled={value.shelf == 'Select'} onPress={() => handlePresentModalPress(ImageIndex.aisle, "Aisle List")}>
                                <Card name={'Aisle'} image={ImageIndex.aisle} height={170} />
                            </TouchableOpacity>
                            {value.aisle !== 'Select' && (<SelectedCard value={value.aisle} handlePress={handlePress1} />)}
                        </View>
                    </View>
                    <View>
                        <ButtonIcon loading={false} name={value.shelf === 'Select' ? 'Select Shelf. . .' : (value.aisle === 'Select' ? 'Select Aisle. . .' : !scannedValue ? "Scan QR" : "Proceed To Click Picture. . .")} width={328} handleSubmit={(value.shelf === 'Select' || value.aisle === 'Select') ? () => handleSubmit('Not Selected') : () => (handleSubmit("Selected"))} />
                    </View>
